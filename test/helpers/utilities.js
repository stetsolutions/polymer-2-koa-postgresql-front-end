module.exports = {
  ...
  shadowSelector: async(page, selector) => {
    ...
    const element = await page.evaluateHandle(
      (elements) => {
        const path = elements[0].getAttribute('data-path').replace('querySelectorAll', 'querySelector')
        ...
        const findNodes = (nodes, parentTags) => {
          ...
          for (let j = 0; j < nodes.length; j++) {
            ...
            if (nodes[j].shadowRoot) {
              ...
              // If found construct the path string
              if (el) {
                let path = ''
                const elements = Object.values(nodes[j].shadowRoot.querySelectorAll(selector))
                ...
                // Append the string with the element as an atrttribute
