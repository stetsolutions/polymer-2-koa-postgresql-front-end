 ...
 * Create example
 *  @async
 *  @param  {string} nameValue value
 *  @param  {string} imagePath value
 */
...
const deleteExample = async() => {
  const deleteButton = await utilities.shadowSelector(page, 'paper-icon-button[icon="delete"]')
  ...
  const yesButton = await page.evaluateHandle(`document.querySelector('#dialog-confirm').querySelector('paper-button[data-option="yes"]')`)
...
test.serial('Should create example successfully', async t => {
  ...
  const editButtons = await utilities.shadowSelectorAll(page, 'paper-icon-button[icon="editor:mode-edit"]')

  // Total number of created examples is equal to the total number of edit icon buttons in the table.
...
test.serial('Should change submit button text when in edit mode', async t => {
  const editButton = await utilities.shadowSelector(page, 'paper-icon-button[icon="editor:mode-edit"]')
...
test.serial('Should clear form successfully', async t => {
  ...
  const fileValidity = await page.evaluate(
    file => file.validity.valid,
    file
  )

  const numOfFiles = await page.evaluate(
...
test.serial('Should contain the correct id in the modal', async t => {
  const tableCellIds = await utilities.shadowSelectorAll(page, '.table__cell--id')
  ...
  const modalMessage = await page.evaluateHandle(`document.querySelector('#dialog-confirm').querySelector('p').querySelector('strong')`)
  ...
  const noButton = await page.evaluateHandle(`document.querySelector('#dialog-confirm').querySelector('paper-button[data-option="no"]')`)

  await page.evaluate(
    noButton => noButton.click(),
    noButton
  )
})
...
test.serial('Should not allow oversized image', async t => {
  ...
  const uploadButtonError = await utilities.waitForShadowSelector(page, '.upload__button--error')

  const errorColor = await page.evaluate(
    uploadButtonError => window.getComputedStyle(uploadButtonError).getPropertyValue('color'),
    uploadButtonError
  )
...
test.serial('Should sort data with name', async t => {
  const columnSort = await utilities.shadowSelectorAll(page, '.table__cell--sort')
...
test.serial('Should sort data with created date', async t => {
  ...
  await t.is(moment(new Date(date1)).isBefore(new Date(date2)), true)
})
